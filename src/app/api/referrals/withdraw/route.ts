...
export async function POST(req: Request) {
  try {
    if (!REFERRALS_ENABLED) {
      return NextResponse.json({ error: "Referrals are currently disabled." }, { status: 403 });
    }
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, commissionBalance: true },
    });
    if (!user) return NextResponse.json({ error: "Account not found." }, { status: 404 });
    if ((user.commissionBalance ?? 0) < value) {
    ...
    console.error("[api/referrals/withdraw] error:", err);
